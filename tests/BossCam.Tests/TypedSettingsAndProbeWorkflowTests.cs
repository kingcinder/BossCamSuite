...
public sealed class TypedSettingsAndProbeWorkflowTests : IDisposable
{
    ...
    public async Task Normalizes_Typed_Settings_From_Snapshot_And_Validation()
    {
        ...
        var typed = new TypedSettingsService(settingsService, new PersistenceVerificationService([new NoopControlAdapter()], store, NullLogger<PersistenceVerificationService>.Instance), new SemanticTrustService(store, BuildContractCatalog(store), settingsService, NullLogger<SemanticTrustService>.Instance), BuildContractCatalog(store), new CapabilityPromotionService(store, BuildContractCatalog(store)), NullLogger<TypedSettingsService>.Instance, new ApplicationStoreTypedControlStore(store));
    ...
    public async Task Allows_Grouped_Writable_Image_Field_Write_Without_Expert_Override()
    {
        ...
        var typed = new TypedSettingsService(settingsService, new PersistenceVerificationService([adapter], store, NullLogger<PersistenceVerificationService>.Instance), new SemanticTrustService(store, BuildContractCatalog(store), settingsService, NullLogger<SemanticTrustService>.Instance), BuildContractCatalog(store), new CapabilityPromotionService(store, BuildContractCatalog(store)), NullLogger<TypedSettingsService>.Instance, new ApplicationStoreTypedControlStore(store));
