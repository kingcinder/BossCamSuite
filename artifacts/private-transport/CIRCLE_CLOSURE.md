...
Final state: CIRCLE_CLOSED_BLOCKED_SDK_MAPPING
...
| Did we extract raw stream, implement SDK relay, or identify exact blocker? | Exact blocker: PRIVATE_TRANSPORT_RELAY_BLOCKED_SDK_MAPPING |
| If relay exists, did we print exact Blue Iris settings? | No relay exists; settings intentionally withheld. |
| Did build/test pass? | Yes: restore PASS, build PASS, test PASS, script parse PASS. |
